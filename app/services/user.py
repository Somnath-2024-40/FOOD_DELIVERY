...
async def _apply_hashed_password(
    data:dict
) ->dict:
    if "password" in data:
...
async def get_users_list(
    ...
    if active_only:
        base = base.where(User.is_axctive.is_(True))
...
async def create_user(
    ...
    exsisting = await _get_user_by_email(db,user.email)
    if exsisting:
        raise HTTPException(
